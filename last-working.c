...
void InitGame(GameContext *ctx)
{
    ...
    for (int i=0; i<5; i++) {
        float maxMovesPerSecond = 3;
        ctx->people[i].liklihoodToMove = maxMovesPerSecond * rand() / RAND_MAX / 60.0f;
        ctx->people[i].liklihoodToTurn = maxMovesPerSecond * rand() / RAND_MAX / 60.0f;
    }
}
    ...
    // Spawn mines and people
        ...
        for (int i=0; i<ctx->mineCount; i++) {
            ...
            while (attempt < max_attempts) {
                ...
                int maxMovesPerSecond = 5;
                ctx->mines[i].liklihoodToMove = maxMovesPerSecond * rand() / RAND_MAX / 60.0f;
                ctx->mines[i].liklihoodToTurn = maxMovesPerSecond * rand() / RAND_MAX / 60.0f;
